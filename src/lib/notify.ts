...
export async function alertProductTargetReached(
  ...
  return sendAlertWithDedup(profile, {
    ...
    message: `🛡️ [SENTINELA] O produto *${productName}* atingiu o preço-alvo!\n\n💵 Preço atual: R$ ${currentPrice}\n🎯 Alvo: R$ ${targetPrice}${link}`,
  });
}
...
export async function alertShoppingItemTargetReached(
  ...
  return notifyAllConfiguredProfilesWithDedup({
    ...
    message: `🛒 [SENTINELA] O item *${itemName}* está dentro do alvo em ${establishmentName}!\n\n💵 Preço: R$ ${price}\n🎯 Alvo: R$ ${targetPrice}\n\nRota otimizada sugerida na aba LOCAL.`,
  });
}
...
export async function alertActivePromotion(
  ...
  return notifyAllConfiguredProfilesWithDedup({
    ...
    message: `🏷️ [SENTINELA] Nova promoção em ${establishmentName}:\n\n📦 *${productName}*\n🔥 De R$ ${regularPrice ?? "—"} por R$ ${promoPrice}${discount}${link}`,
  });
}
