...
- **Ordenação da LISTA DE COMPRAS** — **#45**: 6 modos (`prioridade`/`preco_asc`/`preco_desc`/`az`/`za`/`manual`), coluna `sort_order`, endpoint `POST /shopping-list-items/reorder`, seletor + botões ↑↓ na UI (persistência em localStorage). DIAGNOSTICO §6.30.
