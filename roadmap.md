...
- ~~**ADD em lote: links não encontrados não voltavam (14 → 7, sem bloco para copiar/tentar depois)**~~ — **RESOLVIDO (#53)**: `finishFailures()` marca toda URL que não virou produto (falha/timeout/cancelamento), modal fica **aberto** com banner `X OK • Y FALHARAM` + **bloco `LINKS NÃO ENCONTRADOS (n)`** (URL+erro) com **COPIAR** (URLs puras), **RECOLHER NOS CAMPOS**, **LIMPAR**; persiste em `localStorage` (sobrevive reload) + **COPIAR FALHOS** no SCRAPE LOG. DIAGNOSTICO §6.38.
- ~~**IA sem provedor principal: Gemini 429 crônico + NVIDIA lenta travando compare/scan/scrape/social**~~ — **RESOLVIDO (#54)**: cadeia única **DeepSeek → Gemini → NVIDIA → LM Studio** para tudo (texto, visão e social — busca `googleSearch` do Gemini mantida como fonte de dados, interpretação com DeepSeek primeiro); chave `deepseek_api_key` no perfil (settings AI CORE PARAMETERS, `/api/status`, header), `src/lib/aiProviders.ts` (modelos `deepseek-v4-flash`/`deepseek-v4-flash-vision-exp`, timeout 30s, log `[aiChain]`), `DEEPSEEK_VISION` primeira estratégia da cascata do scraper (screenshot+texto na mesma janela) e LM Studio fechando a cadeia, `runComparison`/`analyze`/`market-search`/social todos reordenados; sem chave = cascata anterior intacta. DIAGNOSTICO §6.39.
- ~~**Tático: varredura da página de ofertas + busca de promoções na hora do ADD**~~ — **RESOLVIDO (#55)**: `src/lib/offerSweep.ts` (render Playwright + cadeia DeepSeek→Gemini, array de promoções sanitizado e salvo em `promotions` com `expires_at` + `SWEEP_TTL_HOURS`) e `src/lib/promoMatch.ts` (casamento promo↔item); `scrapeItemPrice` usa promo vigente sem re-busca, `handleLocalPriceScan` ganha passo de varredura (+`swept`/`promoHits` no job/alerta), MercadoTab mostra PROMOÇÕES/PROMO-CACHE e o ADD avisa loja+preço+validade sem nova busca. DIAGNOSTICO §6.40.
- ~~**Scan Tático: "ERROS 2" — varredura sem IA + cascata `?q=` inútil na página de ofertas**~~ — **RESOLVIDO (#56)**: render multi-captura (até 6 viewports) + cadeia DeepSeek/Gemini vision → text → `extractDetOffers`; `scrapeItemPrice` com flag `offersPage` → promo-cache, senão `notFound` FORA DAS OFERTAS (não-erro) ou error curto da varredura vazia — nunca `?q=`; UI/label `🚫`. DIAGNOSTICO §6.41.
- ~~**Varredura só com Gemini (quota 20/dia) — NVIDIA/LM órfãos + JSON truncado → 0 promoções**~~ — **RESOLVIDO (#57)**: cadeia da varredura ganha **NVIDIA vision** (`meta/llama-3.2-11b-vision-instruct` sondado no catálogo, `max_tokens 3000`, timeout 300s) e **LM Studio por último** (protegido: 2 capturas/1500 tokens/90s); `extractJsonArray` com repair de truncagem (`finish_reason=length`) e preço com vírgula `"price": 22,78` → `"22.78"`; Gemini ganha `abortSignal` 45s (SDK pendurou 6min sem log). Smoke real: **80 promoções** do encarte em 6 capturas/587s, salvas com validade 24h. Sugestão corrigida: `Qwen2.5-VL-3B-Instruct` (3B, não 7B — GTX 960 de 2GB VRAM). DIAGNOSTICO §6.42.
